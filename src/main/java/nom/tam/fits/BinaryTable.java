...
public class BinaryTable extends AbstractTableData implements Cloneable {
    ...
    public static class ColumnDesc implements Cloneable {
        ...
        private void setStringDelimiter(byte delim) {
            if (delim < FitsUtil.MIN_ASCII_VALUE || delim > FitsUtil.MAX_ASCII_VALUE) {
                LOG.warning("WARNING! Substring terminator byte " + (delim & FitsIO.BYTE_MASK) + " outside of the conventional range of " + FitsUtil.MIN_ASCII_VALUE + " through " + FitsUtil.MAX_ASCII_VALUE + " (inclusive)");
            }
            delimiter = delim;
        }
    ...
    private void setNumber(int row, int col, Number value) throws FitsException, ClassCastException {
        ...
        if (q != null) {
            ...
            boolean decimalValue = (value instanceof Float || value instanceof Double || value instanceof BigInteger || value instanceof BigDecimal);
