    ...
    public void prepareUncompressedData(ColumnTable<?> data) throws FitsException {
        ...
        for (int column = 0; column < ncols; column++) {
            for (int tileIndex = 0, rowStart = 0; rowStart < nrows; tileIndex++, rowStart += getRowsPerTile()) {
                BinaryTableTileDescription td = //
                tile().rowStart(//
                rowStart).rowEnd(//
                Math.min(nrows, rowStart + getRowsPerTile())).column(//
                column).tileIndex(//
                tileIndex + 1).compressionAlgorithm(getAlgorithm(column));
                BinaryTableTileCompressor tile = (orig == null) ? new BinaryTableTileCompressor(this, data, td) : new BinaryTableTileCompressor(this, orig, td);
                tiles.add(tile);
            }
        }
        isPrepped = true;
